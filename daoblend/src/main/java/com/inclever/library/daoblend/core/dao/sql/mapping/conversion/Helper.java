...
public class Helper {
	...
	if (date instanceof Timestamp) {
	    calendar.set(Calendar.MILLISECOND,
		    ((Timestamp) date).getNanos() / 1000000);
	}
	return calendar;
    }
	...
	if (!defaultTimeZone.equals(calendar.getTimeZone())) {
	    ...
	    java.sql.Date date = dateFromYearMonthDate(
	...
	return dateFromYearMonthDate(calendar.get(Calendar.YEAR),
     ...
     */
    public static java.sql.Date dateFromYearMonthDate(int year, int month,
	    int day) {
	...
	if (timestamp.getNanos() == 0) {
	    ...
	    nanosString = buildZeroPrefixAndTruncTrailZeros(
		    timestamp.getNanos(), 9);
	}
	...
	if (calendar.get(Calendar.MILLISECOND) == 0) {
	    ...
	    millisString = buildZeroPrefixAndTruncTrailZeros(
	...
	return hourString.concat(StringPool.COLON).concat(minuteString)
		.concat(StringPool.COLON).concat(secondString);
    }
     ...
     */
    public static String buildZeroPrefixAndTruncTrailZeros(int number,
	    int totalDigits) {
	String zeros = "000000000";
	String numbString = Integer.toString(number);

	// Add leading zeros
	numbString = zeros.substring(0, (totalDigits - numbString.length()))
		+ numbString;
     ...
     */
    public static java.sql.Time timeFromString(String timeString)
	    throws DaoConversionException {
	int hour;
	int minute;
	int second;
     ...
     */
    public static java.sql.Time timeFromHourMinuteSecond(int hour, int minute,
	    int second) {
	// Use a calendar to compute the correct millis for the date.
	Calendar localCalendar = allocateCalendar();
	localCalendar.clear();
	...
	if (!defaultTimeZone.equals(calendar.getTimeZone())) {
	    ...
	    Calendar localCalendar = allocateCalendar();
	    localCalendar.setTimeInMillis(calendar.getTimeInMillis());
	    java.sql.Time date = timeFromHourMinuteSecond(
	...
	return timeFromHourMinuteSecond(calendar.get(Calendar.HOUR_OF_DAY),
    ...
    public static java.sql.Timestamp timestampFromString(String timestampString)
	    throws DaoConversionException {
	if ((timestampString.indexOf('-') == -1)
		&& (timestampString.indexOf('/') == -1)
		&& (timestampString.indexOf('.') == -1)
		&& (timestampString.indexOf(':') == -1)) {
	    throw DaoConversionException
		    .incorrectTimestampFormat(timestampString);
	}
	StringTokenizer timestampStringTokenizer = new StringTokenizer(
		timestampString, " /:.-");
	...
	timestamp = new java.sql.Timestamp(year, month, day, hour, minute,
		second, nanos);
	return timestamp;
    }
     ...
     */
    public static java.util.Date utilDateFromTimestamp(
	    java.sql.Timestamp timestampObject) {
	...
	if (appendNanos) {
	    return new java.util.Date(time
		    + (timestampObject.getNanos() / 1000000));
	} else {
	    return new java.util.Date(time);
	}
    }
	...
	byte digit1;
	byte digit2;
	for (byteIndex = bytes.length - 1, strIndex = tmpString.length() - 2; byteIndex >= 0; byteIndex--, strIndex -= 2) {
	    digit1 = (byte) Character.digit(tmpString.charAt(strIndex), 16);
	    digit2 = (byte) Character.digit(tmpString.charAt(strIndex + 1), 16);
	    if ((digit1 == -1) || (digit2 == -1)) {
		throw DaoConversionException.couldNotBeConverted(hex,
	...
	for (int byteIndex = 0; byteIndex < (bytes).length; byteIndex++) {
	    ...
	    if (tempByte > 16) {
		throw DaoConversionException.couldNotBeConverted(bytes,
