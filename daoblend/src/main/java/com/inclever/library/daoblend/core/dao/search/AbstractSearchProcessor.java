...
public abstract class AbstractSearchProcessor {
    ...
    private static Logger logger = LogManagerFactory.getInstance().getLogger(
	    AbstractSearchProcessor.class);
	...
	String select = generateSelectClause(ctx, fields, search.isDistinct(),
		search.isConstructorExpressionQuery(),
    ...
    protected String generateSelectClause(SearchContext ctx,
	    ...
	    String constructorExpressionClassName) {
	...
	if (fields != null) {
		...
		if (first) {
			...
			sb.append("NEW ")
				.append(constructorExpressionClassName)
				.append(" ( ");
		    }
		    first = false;
		} else {
	...
	for (Sort sort : sorts) {
		...
		appendCustomExpression(sb, ctx, sort.getProperty());
		    ...
		    && metadataUtil.get(ctx.rootClass, sort.getProperty())
			    .isString()) {
		sb.append("lower(");
	...
	if (filters == null || filters.size() == 0) {
	    ...
	    Filter junction = new Filter(null, filters,
		    isDisjunction ? Operator.OP_OR : Operator.OP_AND);
	...
	case OP_OR:
	    ...
	    String op = filter.getOperator() == Operator.OP_AND ? " and "
		    : " or ";
	...
	ctx2.rootClass = metadataUtil.get(ctx.rootClass, property)
		.getJavaClass();
	...
	if (property != null
	    ...
	    expectedClass = metadataUtil.get(rootClass, property)
		    .getJavaClass();
	}
	...
	if (pos == -1) {
		...
		if (metadataUtil.isId(ctx.rootClass, currentPath)) {
			...
			&& metadataUtil.get(ctx.rootClass, currentPath)
				.isEntity()) {
		...
		if (pos != -1
			...
			&& metadataUtil.get(ctx.rootClass,
				currentPath.substring(0, pos)).isCollection()) {
		    first = true;
		}
     ...
     */
    protected AliasNode getAlias(SearchContext ctx, String path,
	    boolean setFetch) {
