 ...
 * <code>ISearch</code>.
 * <ol>
 * <li><code>generateQL()</code> - is used for getting the actual search result.
 * </li>
...
public abstract class AbstractSearchProcessor {
    ...
    private static final Logger logger = LogManagerFactory.getInstance().getLogger(AbstractSearchProcessor.class);
    ...
    public String generateQL(Class<?> entityClass, ISearch search, List<Object> paramList) {
        ...
        String select = generateSelectClause(ctx, fields, search.isDistinct(), search.isConstructorExpressionQuery(),
                search.getConstructorExpressionClass());
        ...
        logger.debug("Generated Query: {} {}", StringPool.NEW_LINE, query);
        return query;
    }

    /**
     * Generate the QL string that will query the total number of result from a
    ...
    protected String generateSelectClause(SearchContext ctx, List<Field> fields, boolean distinct,
            boolean isConstructorExpressionQuery, String constructorExpressionClassName) {
        ...
        if (fields != null) {
            for (Field field : fields) {
                if (first) {
                    ...
                    if (distinct) {
                        sb.append("distinct ");
                    } else if (isConstructorExpressionQuery) {
                        sb.append("NEW ").append(constructorExpressionClassName).append(" ( ");
                    }
                    first = false;
                } else {
                    sb.append(", ");
        ...
        if (isConstructorExpressionQuery) {
            sb.append(" ) ");
        }
        if (useOperator && notUseOperator) {
            throw new Error("A search can not have a mix of fields with operators and fields without operators.");
    ...
    protected String generateWhereClause(SearchContext ctx, List<Filter> filters, boolean isDisjunction) {
        ...
        if (filters == null || filters.size() == 0) {
            ...
            Filter junction = new Filter(null, filters, isDisjunction ? Operator.OP_OR : Operator.OP_AND);
    ...
    protected String filterToQL(SearchContext ctx, Filter filter) {
        ...
        // respectively
        if (operator == Operator.OP_IN || operator == Operator.OP_NOT_IN) {
            if (value instanceof Collection && ((Collection) value).size() == 0) {
                return operator == Operator.OP_IN ? "1 = 2" : "1 = 1";
            }
        ...
        case OP_OR:
            ...
            String op = filter.getOperator() == Operator.OP_AND ? " and " : " or ";
    ...
    protected String generateSimpleAllOrSome(SearchContext ctx, String property, Filter filter, String operation) {
        ...
        Object value = SearchUtil.convertIfNeeded(filter.getValue(),
                metadataUtil.get(ctx.rootClass, property).getJavaClass());
        ...
        if (isCollection) {
            ...
            if (value instanceof Collection) {
                ...
                for (Object item : (Collection) value) {
                    val2[i++] = SearchUtil.convertIfNeeded(item, expectedClass);
                }
            } else {
            ...
            return SearchUtil.convertIfNeeded(value, expectedClass);
        }
    }
            ...
            if (expression.charAt(matcher.start()) == '{') {
                ...
                if (values == null || values.isEmpty()) {
                    throw new IllegalArgumentException(
                            "This custom filter expression (" + expression + ") calls for a value placeholder number "
                                    + valueIndex + ", but no values were specified.");
                }
                if (valueIndex > values.size()) {
                    throw new IllegalArgumentException(
                            "This custom filter expression (" + expression + ") calls for a value placeholder number "
                                    + valueIndex + ", but only " + values.size() + " values were specified.");
                }
    ...
    protected List<Filter> checkAndCleanFilters(List<Filter> filters) {
        return SearchUtil.walkFilters(filters, new SearchUtil.FilterVisitor() {
            ...
            public Filter visitAfter(Filter filter) {
                ...
                if (filter.getOperator() == Operator.OP_CUSTOM) {
