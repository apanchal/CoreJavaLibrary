...
public class JdbcUtil {
     ...
     * specified. Valid values are those specified by
     * the @{java.sql.DatabaseMetaData#getTables} method's TABLE_TYPE column.
     * The default only includes standard, user-defined tables.
     */
    public static final Set<String> DEFAULT_TABLE_TYPES = Collections
            .unmodifiableSet(new HashSet<>(Arrays.asList("TABLE")));
    ...
    private static final int GET_COLUMNS_COLUMN_TYPE = 5;
     ...
     * which only include user-defined tables.
     * 
     * @param conn
     *            database connection
     ...
     *            database connection
     * @param types
     *            a set of table types that should be included in the results
     * @throws SQLException
     */
        ...
        try (ResultSet rs = metadata.getTables(null, null, "%", null)) {
            ...
            while (rs.next()) {
                if (types.contains(rs.getString(GET_TABLES_TYPE_COLUMN))) {
                    ...
                    // SQLite JDBC driver does not correctly mark these as
                    // system tables
     ...
     *         if there is no autoincrement column or more than one exists
     * @throws SQLException
     */
    public static String getAutoincrementColumn(Connection conn, String table) throws SQLException {
        ...
        // empty table
        LOGGER.trace("Falling back to SELECT detection of auto-increment column for {}:{}", conn, table);
    ...
    public static Timestamp getCurrentTimeOnDB(Connection conn, Calendar cal) throws SQLException, ConnectException {
        ...
        // This is ugly, but to run a function, everyone does 'select
        // function()'
        // except Oracle that does 'select function() from dual'
        // and Derby uses either the dummy table SYSIBM.SYSDUMMY1 or values
        // expression (I chose to use values)
     ...
     * @throws SQLException
     */
    public static List<TableColumn> getColumes(Connection conn, String tableName) throws SQLException {
        List<TableColumn> columns = new Vector<>();
        if (conn != null && tableName != null && !tableName.isEmpty()) {
            TableColumn column;
            try (ResultSet rs = conn.getMetaData().getColumns(null, null, tableName, "%")) {
                while (rs.next()) {

                    column = new TableColumn(rs.getString(GET_COLUMNS_COLUMN_NAME));
                    column.setType(rs.getString("TYPE_NAME"));
                    column.setSize(rs.getInt("COLUMN_SIZE"));
                    String isAutoIncKey = rs.getString("IS_AUTOINCREMENT").equals("YES") ? "true" : "false";
                    column.setAutoIncrement(new Boolean(isAutoIncKey));
                    columns.add(column);
                }
            } catch (Exception e) {
